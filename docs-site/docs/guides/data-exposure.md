...
reach them. That is why a glyf dashboard costs nothing to host, and it makes
*what is in this page* the question to ask before publishing it.
...
boundary the data crosses. People mean three different things by the phrase:
...
through a SQL client, and it has the same answer: warehouse governance. glyf's
own controls sit behind the role ceiling, for rows that were legitimately
readable but should not travel.

## What a default export contains

| Published file | Carries |
| --- | --- |
| `dashboards/*.html` | The rows, twice over. See below. |
...
Two of those are not visible from reading the dashboard.
...
This keeps every chart as it is, interactive charts included, and strips what
the picture does not show. An interactive chart's `datasets` block is pruned to
the columns its `VISUALISE` clause encodes, so a query that selects
`customer_id` alongside the two columns it plots no longer publishes
...
Values are never rounded or otherwise transformed. The mode prunes columns and
touches nothing else. **A `minimal` export contains no information beyond what
the rendered chart displays.** The encoded values are still there, because Vega
...
that number. The mode withholds the rows and not the conclusions drawn from
them.
...
A column is PII if `privacy.pii_columns` lists it, or if the dbt project tags
it with `meta: {pii: true}` or `tags: [pii]` in `schema.yml`, on a model or
source the chart reads. Under `deny`, the default, a chart whose query returns
such a column fails the build, in validate mode too, and the message names the
column and where the classification came from. Under `redact`, the
...
has the details and the limits. Classification is by column name, so an alias
needs listing in `glyf.yml`.
...
excluded. Nothing reads it back or enforces it.
...
them, or, for an interactive chart, pull them out of the page. `row_data:
minimal` keeps the page from holding anything beyond what it displays. The rest
is deciding who may open it, and, in a hosted setting, watermarking and audit
so that extraction is attributable rather than prevented.
