...
def run_init(
    ...
    *,
    ...
    typer.echo(f"Initialized glyf in {_rel(root, Path.cwd())}")
...
def _config_file(root: Path, config_path: Path | None) -> Path:
    if config_path is None:
        return root / "glyf.yml"
    expanded = config_path.expanduser()
    return expanded if expanded.is_absolute() else root / expanded


def _load_or_default_config(root: Path, config_file: Path) -> tuple[GlyfConfig, bool]:
    if not config_file.exists():
        return GlyfConfig(), False
...
def _starter_ggsql(*, model_name: str, chart_title: str, chart_type: str) -> str:
    return (
        ...
        'LABEL subtitle => "Starter chart generated by glyf init"\n'
        'LABEL x_title => "Date"\n'
        'LABEL y_title => "Value"\n'
        "CONFIG width => 900\n"
        "CONFIG height => 500\n"
